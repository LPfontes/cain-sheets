...
const DICE = (function() {
    ...
    that.dice_box.prototype.reinit = function(container) {
        ...
        vars.scale = vars.scale * Math.sqrt(this.w * this.w + this.h * this.h) / 5; // Mude o / 5 para alterar o tamanho (menor divisor = dados maiores)
    ...
    threeD_dice.create_d6 = function() {
        ...
        if (!this.d6_material) this.d6_material = new THREE.MeshFaceMaterial(
                create_dice_materials([' ', ' ', '1', '2', '3', '4', '5', '6', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '], vars.scale / 2, 1.0));
    ...
    threeD_dice.create_d9 = function() {
        ...
        if (!this.d9_material) this.d9_material = new THREE.MeshFaceMaterial(
                create_dice_materials(CONSTS.standart_d20_dice_face_labels, vars.scale / 2, 1.0));
        return new THREE.Mesh(this.d10_geometry, this.d9_material);
    }

    threeD_dice.create_d10 = function() {
        if (!this.d10_geometry) this.d10_geometry = create_d10_geometry(vars.scale * 0.9);
        if (!this.d10_material) this.d10_material = new THREE.MeshFaceMaterial(create_custom_d10_materials());
        return new THREE.Mesh(this.d10_geometry, this.d10_material);
    }


    threeD_dice.create_d12 = function() {
        if (!this.d12_geometry) this.d12_geometry = create_d12_geometry(vars.scale * 1.1);
        if (!this.d12_material) this.d12_material = new THREE.MeshFaceMaterial(create_custom_d12_materials());
    ...
    function create_dice_materials(face_labels, size, margin) {
        ...
        for (var i = 0; i < face_labels.length; ++i) {
            var opts = $t.copyto(vars.material_options, {});
            opts.map = create_text_texture(face_labels[i], vars.label_color, vars.dice_color);
            opts.color = 0xffffff;
            materials.push(new THREE.MeshPhongMaterial(opts));
        }
        return materials;
    }
    ...
    function create_d4_materials(size, margin, labels) {
        ...
        for (var i = 0; i < labels.length; ++i) {
            var opts = $t.copyto(vars.material_options, {});
            opts.map = create_d4_text(labels[i], vars.label_color, vars.dice_color);
            opts.color = 0xffffff;
            materials.push(new THREE.MeshPhongMaterial(opts));
        }
        return materials;
    ...
    function create_d10_geometry(radius) {
        ...
        var tab = 0.2; // Ajuste essa margem para afastar a imagem das bordas
        var geom = create_geom(vertices, faces, radius, tab, Math.PI, 0.945);
        
        var scale = 1 / (1 + tab);
        function mapUV(u, v) {
            return new THREE.Vector2(0.5 + (u - 0.5) * scale, 0.5 + (v - 0.5) * scale);
        }
        ...
        for (var i = 0; i < 10; ++i) {
            ...
            geom.faceVertexUvs[0][i * 2] = uvs1;
            geom.faceVertexUvs[0][i * 2 + 1] = uvs2;
        }
        return geom;
    }
    ...
    function create_d12_geometry(radius) {
        ...
        return create_geom(vertices, faces, radius, 0.6, Math.PI / 10, 0.968);
    }
    ...
    function playSound(outerContainer, soundVolume) {
        ...
        audio.src = 'dice/assets/nc93322.mp3'; //todo: make this configurable
